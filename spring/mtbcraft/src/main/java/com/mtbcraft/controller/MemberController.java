...
public class MemberController {
	...
	public String joinPOST(HttpServletRequest request, @RequestParam("userphoto") MultipartFile profile) throws Exception{
		...
		//String path = "C:\\Users\\woolu\\Desktop\\workspace\\data\\rider";
		...
		if(member.getR_type().equals("1")) {
			Repair_Shop rs = new Repair_Shop();
			rs.setRs_id(member.getR_id());
			rs.setRs_info(request.getParameter("rs_info"));
			rs.setRs_holiday(request.getParameter("rs_holiday"));
			rs.setRs_lat(request.getParameter("rs_lat"));
			rs.setRs_lon(request.getParameter("rs_lon"));
			
			memberService.regRepair_Shop(rs);
		}
		
		return "redirect:/";
	}
	// 정비소 회원가입
	@RequestMapping(value="/member/repair_join")
	public String repair_join() {
		return "/member/repairjoin";
	}
